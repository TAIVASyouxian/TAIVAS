...
TAIVAS_VERSION = "V10.2 Testing Core - International Alignment + Wildfire Energy-Impact Layer"
...
# V10.2: independent energy calculation core; no dependency on taivas_core.energy_model.compute_energy_supply
from taivas_core.utils import clamp, safe_div
from taivas_core.trend_forecast import (
    safe_float,
    safe_int,
    sort_history_df,
    prepare_uploaded_preview,
    build_forecast_chart_df,
    compute_reference_average,
    build_energy_contribution_df,
    compute_trend_estimates,
)
from taivas_core.ui_render import (
    mini_card,
    page_question,
    concept_badge,
    render_capacity_factor_chart,
    render_delta_chart,
    render_critical_load_chart,
...
PRODUCT_TIERS = {
    "Demo": {
        "label": "Demo",
        "positioning": "Public concept demo for quick review and first contact.",
        "workspaces": ["Executive Dashboard"],
        "limits": ["Simplified scenario review", "CSV upload allowed", "Decision-support only"],
        "best_for": "ITRI first review, public prototype, early partner conversation",
    },
    "Pro": {
        "label": "Pro / Analyst",
        "positioning": "Analyst tier for deeper scenario comparison and operational resilience testing.",
        "workspaces": ["Executive Dashboard", "Analyst Workspace"],
        "limits": ["Scenario comparison", "Forecast and confidence band", "Export center"],
        "best_for": "Energy analysts, resilience planners, internal evaluation teams",
    },
    "Enterprise": {
        "label": "Institution / Enterprise",
        "positioning": "Enterprise-oriented tier for critical facilities, governance notes, and deployment planning.",
        "workspaces": ["Executive Dashboard", "Analyst Workspace", "Concept Lab"],
        "limits": ["Critical facility logic", "Governance guardrails", "Docker deployment package"],
        "best_for": "Hospitals, long-term care groups, campuses, municipalities, infrastructure partners",
    },
}

def render_product_tier_panel(product_tier: str):
    tier = PRODUCT_TIERS.get(product_tier, PRODUCT_TIERS["Demo"])
    st.markdown(
        f"""
        <div class="hero" style="margin-top:14px;">
          <h3>Product Tier: {tier["label"]}</h3>
          <p>{tier["positioning"]}</p>
          <p><b>Best for:</b> {tier["best_for"]}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    tier_cols = st.columns(len(PRODUCT_TIERS))
    for col, (name, info) in zip(tier_cols, PRODUCT_TIERS.items()):
        active_badge = "Active" if name == product_tier else "Available"
        items = "<br>".join([f"• {x}" for x in info["limits"]])
        with col:
            st.markdown(
                f"""
                <div class="layer-box">
                  <div class="card-label">{active_badge}</div>
                  <div class="card-value" style="font-size:1rem; margin-bottom:8px;">{info["label"]}</div>
                  <div style="font-size:0.88rem; opacity:0.90; line-height:1.55;">{items}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
...
ONBOARDING_DEMO_CASES = {
    "Taipei Typhoon Test": {
        "country": "Taiwan",
        "city": "Taipei",
        "scenario": "typhoon",
        "purpose": "Stress-test an island city under storm damage, solar loss, and reserve pressure.",
        "what_to_read": ["Shortfall", "Grid Dependency", "Survival Timeline", "Energy Security"],
    },
    "Helsinki Blizzard Test": {
        "country": "Finland",
        "city": "Helsinki",
        "scenario": "blizzard",
        "purpose": "Review winter reliability, heating load pressure, and reserve endurance.",
        "what_to_read": ["Demand", "Final Supply", "Critical Failure Hour", "Reserve Outlook"],
    },
    "Berlin Heatwave Test": {
        "country": "Germany",
        "city": "Berlin",
        "scenario": "heat_wave",
        "purpose": "Compare cooling-load growth and renewable contribution under heat stress.",
        "what_to_read": ["Demand delta", "Renewable Supply", "Scenario Comparison", "Forecast Trend"],
    },
    "Reykjavik Storm Test": {
        "country": "Iceland",
        "city": "Reykjavik",
        "scenario": "storm",
        "purpose": "Explore geothermal-heavy resilience with storm-driven renewable variability.",
        "what_to_read": ["Dominant Source", "Actual Mix", "Stress Test", "Timeline"],
    },
}
...
DEMO_PRESET_DEFAULTS = {
    "Taiwan Typhoon": {"country": "Taiwan", "city": "Taipei", "population": 2500000, "scenario_key": "typhoon", "temperature": 29, "wind_speed": 18.0, "solar_radiation": 180, "precipitation": 160, "humidity": 92, "solar_failure_ratio": 0.25, "wind_failure_ratio": 0.18, "hydro_failure_ratio": 0.12, "battery_failure_ratio": 0.08, "energy_security_scenario": "regional_disruption"},
    "Finland Blizzard": {"country": "Finland", "city": "Helsinki", "population": 664000, "scenario_key": "blizzard", "temperature": -12, "wind_speed": 14.0, "solar_radiation": 90, "precipitation": 80, "humidity": 84, "solar_failure_ratio": 0.35, "wind_failure_ratio": 0.16, "hydro_failure_ratio": 0.08, "battery_failure_ratio": 0.10, "energy_security_scenario": "normal"},
    "Germany Energy Security": {"country": "Germany", "city": "Berlin", "population": 3570000, "scenario_key": "cold_wave", "temperature": 1, "wind_speed": 5.5, "solar_radiation": 220, "precipitation": 28, "humidity": 78, "import_dependency": 0.62, "shipping_dependency": 0.55, "infrastructure_damage_ratio": 0.12, "reserve_recovery_lag_days": 7, "energy_security_scenario": "regional_disruption"},
...
def activate_demo_preset(preset_name: str):
    st.session_state["demo_mode_autofill"] = preset_name
    st.rerun()

def render_onboarding_panel(product_tier: str):
    st.markdown(
        """
        <div class="hero" style="border:1px solid rgba(59,130,246,0.32); margin-bottom:14px;">
          <h3>Welcome to TAIVAS</h3>
          <p><b>TAIVAS</b> is an energy resilience decision-support simulator for extreme climate, energy security, and critical-facility continuity scenarios.</p>
          <p style="opacity:0.88;">Use this prototype to compare scenarios, inspect shortfall risk, review renewable contribution, and generate export-ready summaries for discussion.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    quick_steps = [
        ("1", "Select city", "Choose the country and city model."),
        ("2", "Choose scenario", "Pick normal, heat wave, storm, cold wave, blizzard, or typhoon."),
        ("3", "Read dashboard", "Check demand, supply, shortfall, efficiency, and grid dependency."),
        ("4", "Export / discuss", "Download CSV, summary, or audit trail for review."),
    ]
    for col, (num, title, body) in zip(qs_cols, quick_steps):
        with col:
            st.markdown(
                f"""
                <div class="layer-box">
                  <div class="card-label">Quick Start {num}</div>
                  <div class="card-value" style="font-size:1rem;">{title}</div>
                  <div style="font-size:0.86rem; opacity:0.86; line-height:1.45;">{body}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    with st.expander("Guided demo scenarios for first-time reviewers", expanded=False):
        demo_cols = st.columns(2)
        for idx, (name, case) in enumerate(ONBOARDING_DEMO_CASES.items()):
            with demo_cols[idx % 2]:
                reads = ", ".join(case["what_to_read"])
                st.markdown(
                    f"""
                    <div class="layer-box" style="margin-bottom:10px;">
                      <div class="card-label">Suggested demo</div>
                      <div class="card-value" style="font-size:1rem;">{name}</div>
                      <div style="font-size:0.88rem; line-height:1.55; opacity:0.90;">
                        <b>Scenario:</b> {case.get('country', '-')} / {case['city']} / {case['scenario']}<br>
                        <b>Purpose:</b> {case['purpose']}<br>
                        <b>Read:</b> {reads}
                      </div>
                    </div>
                    """,
                ...
                if st.button(f"Apply {name}", key=f"apply_demo_{idx}"):
                    activate_demo_preset(name)
    st.info(
        "For quick testing, use the sidebar Demo Mode presets. Product Tier controls how much of the SaaS prototype is exposed: Demo, Pro / Analyst, or Institution / Enterprise."
    )
    st.warning(
        "Model limitation: TAIVAS is a decision-support and scenario-exploration tool. It is not a guaranteed prediction system and does not replace engineering validation, grid-operator data, or formal risk assessment."
    )
...
def render_trend_estimate_panel(trend_df, multistep_df, trend_meta):
    ...
    render_forecast_chart(multistep_df, tr, build_forecast_chart_df)
...
# Backward-compatible metadata defaults. These are not the V10.2 formulas;
# they only keep older UI/config code from failing when new scenarios are added.
DEFAULT_SCENARIO_METADATA = {"demand": 1.0, "solar": 1.0, "wind": 1.0, "hydro": 1.0}
for _scenario_name, _scenario_meta in SCENARIOS.items():
    for _k, _v in DEFAULT_SCENARIO_METADATA.items():
        _scenario_meta.setdefault(_k, _v)
...
def compute_energy_supply(inputs, scenario_key, failure_ratios=None, reserve_recovery_lag_days=0):
    """Independent V10.2 calculation core.

    This function does not call the older taivas_core.energy_model.compute_energy_supply.
    It keeps the surrounding dashboard compatible while replacing the energy math with
    internationally aligned, simplified decision-support formulas.
    """
    failure_ratios = failure_ratios or {}
    scenario_key = str(scenario_key or "normal")

    solar_capacity = max(safe_float(inputs.get("solar_capacity"), 0.0), 0.0)
    wind_capacity = max(safe_float(inputs.get("wind_capacity"), 0.0), 0.0)
    geothermal_capacity = max(safe_float(inputs.get("geothermal_capacity"), 0.0), 0.0)
    hydro_capacity = max(safe_float(inputs.get("hydro_capacity"), 0.0), 0.0)
    battery_capacity = max(safe_float(inputs.get("battery_capacity"), 0.0), 0.0)
    population = max(safe_float(inputs.get("population"), 100000.0), 1.0)

    solar = _pvwatts_solar_output(
        solar_capacity,
        inputs.get("solar_radiation", 0),
        inputs.get("temperature", 20),
        scenario_key,
        failure_ratios.get("solar", 0.0),
    )
    wind = _wind_power_curve_output(
        wind_capacity,
        inputs.get("wind_speed", 0),
        scenario_key,
        failure_ratios.get("wind", 0.0),
    )
    geothermal = _firm_output(
        geothermal_capacity,
        "geothermal",
        scenario_key,
        failure_ratios.get("geothermal", 0.0),
        inputs.get("precipitation", 0),
    )
    hydro = _firm_output(
        hydro_capacity,
        "hydro",
        scenario_key,
        failure_ratios.get("hydro", 0.0),
        inputs.get("precipitation", 0),
    )
    ...
    installed_mix_mw = {
        ...
        "Geothermal": round(geothermal_capacity, 2),
        "Hydro": round(hydro_capacity, 2),
    }
    total_installed = sum(installed_mix_mw.values())
    renewable_supply = sum(actual_mix_mw.values())

    installed_mix_pct = {
        k: round(safe_div(v, total_installed) * 100 if total_installed > 0 else 0.0, 2)
        for k, v in installed_mix_mw.items()
    }
    actual_mix_pct = {
        k: round(safe_div(v, renewable_supply) * 100 if renewable_supply > 0 else 0.0, 2)
        for k, v in actual_mix_mw.items()
    }
    capacity_factors = {
        "Solar": round(safe_div(solar, solar_capacity) * 100 if solar_capacity > 0 else 0.0, 2),
        "Wind": round(safe_div(wind, wind_capacity) * 100 if wind_capacity > 0 else 0.0, 2),
        "Geothermal": round(safe_div(geothermal, geothermal_capacity) * 100 if geothermal_capacity > 0 else 0.0, 2),
        "Hydro": round(safe_div(hydro, hydro_capacity) * 100 if hydro_capacity > 0 else 0.0, 2),
    }
    dominant_source = max(actual_mix_mw, key=actual_mix_mw.get) if actual_mix_mw else "N/A"

    # V10.2 demand baseline: population-scaled city/facility proxy.
    # The weather adjustment is degree-day style and scenario-aware.
    base_demand = max(population / 100000.0 * 8.0, 1.0)
    ...
    # Event-to-energy impact layer. Wildfire is intentionally NOT a fire-spread model;
    # it only translates wildfire-like conditions into energy-system stress.
    ...
    if scenario_key == "storm":
        ...
        transmission_derate *= 0.94
    elif scenario_key == "wildfire":
        transmission_derate *= 0.78

    usable_supply_before_storage = renewable_supply * transmission_derate

    # Standard storage energy-balance style support for the current scenario step.
    battery_available = battery_capacity * (1.0 - clamp(failure_ratios.get("battery", 0.0), 0.0, 1.0))
    battery_available *= 0.90  # simplified round-trip / operational derate
    immediate_gap = max(demand - usable_supply_before_storage, 0.0)
    battery_discharge_support = min(battery_available * 0.25, immediate_gap)
    final_supply = usable_supply_before_storage + battery_discharge_support
    shortfall = max(demand - final_supply, 0.0)
    battery_remaining = max(battery_available - battery_discharge_support, 0.0)
    ...
    system_efficiency = round(
        clamp(100.0 - safe_div(shortfall, demand) * 100.0 - (1.0 - transmission_derate) * 20.0, 0.0, 100.0),
        2,
    )
    reserve_days_remaining = round(max(battery_remaining / max(demand * 24.0, 1.0), 0.0), 2)
    recovery_time_estimate = round(max(1.0, reserve_recovery_lag_days + grid_dependency / 20.0 + (1.0 - transmission_derate) * 5.0), 1)
    ...
    return {
        ...
        "installed_mix_mw": installed_mix_mw,
        "installed_mix_pct": installed_mix_pct,
        "actual_mix_mw": actual_mix_mw,
        ...
        "v10_2_core_mode": "independent_testing_core",
        "wildfire_scope_note": "Wildfire is modeled only as an energy-system impact trigger: smoke attenuation, transmission disruption, and grid-isolation stress. TAIVAS does not model fire spread or evacuation zones.",
    }
...
def render_model_alignment_panel(results):
    ...
    st.markdown(
        """
        <div class="note">
        TAIVAS V10 uses simplified but internationally aligned modeling concepts: PVWatts/SAM-style PV derating, IEC-style wind power-curve behavior, degree-day demand sensitivity, and battery energy-balance logic. The purpose is external testing and scenario comparison, not certified engineering design or guaranteed prediction.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    product_tier = st.selectbox(
        "Product Tier",
        list(PRODUCT_TIERS.keys()),
        index=0,
        help="Demo limits the app to a public-facing executive view. Pro unlocks analyst testing. Enterprise unlocks concept lab and deployment/governance framing.",
    )
    st.caption(PRODUCT_TIERS[product_tier]["positioning"])

    requested_demo_mode = st.session_state.pop("demo_mode_autofill", st.session_state.get("demo_mode_selection", "Manual"))
    demo_index = DEMO_MODE_OPTIONS.index(requested_demo_mode) if requested_demo_mode in DEMO_MODE_OPTIONS else 0
    ...
    preset_sidebar = DEMO_PRESET_DEFAULTS.get(demo_mode, {})
    if demo_mode != "Manual":
        st.success(f"Demo preset active: {demo_mode}")
        st.caption("Sidebar inputs below are pre-filled from the selected demo preset.")
    ...
    default_country = preset_sidebar.get("country") or (uploaded_profile["country"] if (use_uploaded and uploaded_profile) else list(merged_city_data.keys())[0])
    ...
    country = st.selectbox(tr("country"), country_options, index=country_index, disabled=(use_uploaded and uploaded_profile is not None), key=f"country_select_{demo_mode}")

    city_options = list(merged_city_data[country].keys())
    default_city = preset_sidebar.get("city") if preset_sidebar.get("country") == country else None
    default_city = default_city or (uploaded_profile["city"] if (use_uploaded and uploaded_profile and uploaded_profile["country"] == country) else city_options[0])
    city_index = city_options.index(default_city) if default_city in city_options else 0
    city = st.selectbox(tr("city"), city_options, index=city_index, disabled=(use_uploaded and uploaded_profile is not None), key=f"city_select_{demo_mode}")
    ...
    population_default = int(preset_sidebar.get("population", active_population))
    population = st.slider(tr("population"), 10000, 5000000, int(clamp(population_default, 10000, 5000000)), step=10000, key=f"population_slider_{demo_mode}")
    ...
    temperature_default = preset_sidebar.get("temperature", uploaded_profile["temperature"] if (use_uploaded and uploaded_profile) else 26)
    temperature = st.slider(tr("temperature") + " (°C)", -20, 50, int(clamp(temperature_default, -20, 50)), 1, key=f"temperature_slider_{demo_mode}")
    wind_speed_default = preset_sidebar.get("wind_speed", uploaded_profile["wind_speed"] if (use_uploaded and uploaded_profile) else 4.2)
    wind_speed = st.slider(tr("wind_speed") + " (m/s)", 0.0, 30.0, float(clamp(wind_speed_default, 0.0, 30.0)), 0.1, key=f"wind_speed_slider_{demo_mode}")
    solar_radiation_default = preset_sidebar.get("solar_radiation", uploaded_profile["solar_radiation"] if (use_uploaded and uploaded_profile) else 640)
    solar_radiation = st.slider(tr("solar_radiation") + " (W/m²)", 0, 1200, int(clamp(solar_radiation_default, 0, 1200)), 10, key=f"solar_radiation_slider_{demo_mode}")
    precipitation_default = preset_sidebar.get("precipitation", uploaded_profile["precipitation"] if (use_uploaded and uploaded_profile) else 12)
    precipitation = st.slider(tr("precipitation") + " (mm)", 0, 300, int(clamp(precipitation_default, 0, 300)), 1, key=f"precipitation_slider_{demo_mode}")
    humidity_default = preset_sidebar.get("humidity", uploaded_profile["humidity"] if (use_uploaded and uploaded_profile) else 73)
    humidity = st.slider(tr("humidity") + " (%)", 0, 100, int(clamp(humidity_default, 0, 100)), 1, key=f"humidity_slider_{demo_mode}")
    scenario_options = list(SCENARIOS.keys())
    scenario_default = preset_sidebar.get("scenario_key", scenario_options[0])
    scenario_index = scenario_options.index(scenario_default) if scenario_default in scenario_options else 0
    scenario_key = st.selectbox(tr("weather_scenario"), scenario_options, index=scenario_index, key=f"scenario_select_{demo_mode}")

    st.divider()
    st.subheader(tr("stress_inputs"))
    solar_failure_ratio = st.number_input(tr("solar_failure_ratio"), 0.0, 1.0, float(preset_sidebar.get("solar_failure_ratio", 0.00)), 0.05, format="%.2f", key=f"solar_failure_{demo_mode}")
    wind_failure_ratio = st.number_input(tr("wind_failure_ratio"), 0.0, 1.0, float(preset_sidebar.get("wind_failure_ratio", 0.00)), 0.05, format="%.2f", key=f"wind_failure_{demo_mode}")
    geothermal_failure_ratio = st.number_input(tr("geothermal_failure_ratio"), 0.0, 1.0, 0.00, 0.05, format="%.2f")
    hydro_failure_ratio = st.number_input(tr("hydro_failure_ratio"), 0.0, 1.0, float(preset_sidebar.get("hydro_failure_ratio", 0.00)), 0.05, format="%.2f", key=f"hydro_failure_{demo_mode}")
    battery_failure_ratio = st.number_input(tr("battery_failure_ratio"), 0.0, 1.0, float(preset_sidebar.get("battery_failure_ratio", 0.00)), 0.05, format="%.2f", key=f"battery_failure_{demo_mode}")

    st.divider()
    st.subheader(tr("security_inputs"))
    security_options = list(ENERGY_SECURITY_SCENARIOS.keys())
    security_default = preset_sidebar.get("energy_security_scenario", security_options[0])
    security_index = security_options.index(security_default) if security_default in security_options else 0
    energy_security_scenario = st.selectbox(tr("energy_security_scenario"), security_options, index=security_index, key=f"security_select_{demo_mode}")
    import_dependency = st.number_input(tr("import_dependency"), 0.0, 1.0, float(preset_sidebar.get("import_dependency", 0.70)), 0.01, format="%.2f", key=f"import_dependency_{demo_mode}")
    strategic_reserve_days = st.number_input(tr("strategic_reserve_days"), 0, 365, 20, 1)
    shipping_dependency = st.number_input(tr("shipping_dependency"), 0.0, 1.0, float(preset_sidebar.get("shipping_dependency", 0.85)), 0.01, format="%.2f", key=f"shipping_dependency_{demo_mode}")
    infrastructure_damage_ratio = st.number_input(tr("infrastructure_damage_ratio"), 0.0, 1.0, float(preset_sidebar.get("infrastructure_damage_ratio", 0.10)), 0.01, format="%.2f", key=f"infrastructure_damage_{demo_mode}")
    reserve_recovery_lag_days = st.number_input(tr("reserve_recovery_lag_days"), 0, 30, int(preset_sidebar.get("reserve_recovery_lag_days", 3)), 1, key=f"reserve_lag_{demo_mode}")
...
if demo_mode != "Manual":
    demo_presets = {
        ...
        "Taipei Typhoon Test": {
            ...
            "solar_radiation": 180, "precipitation": 160, "humidity": 92,
            "solar_failure_ratio": 0.25, "wind_failure_ratio": 0.18, "hydro_failure_ratio": 0.12, "battery_failure_ratio": 0.08,
            "energy_security_scenario": "regional_disruption",
            "enable_geopolitical_shock": False,
        },
        "Helsinki Blizzard Test": {
            "country": "Finland", "city": "Helsinki", "population": 664000,
        ...
        "Berlin Heatwave Test": {
            "country": "Germany", "city": "Berlin", "population": 3570000,
            "scenario_key": "heat_wave", "temperature": 38, "wind_speed": 4.2,
            "solar_radiation": 920, "precipitation": 4, "humidity": 56,
            "solar_failure_ratio": 0.04, "wind_failure_ratio": 0.08, "hydro_failure_ratio": 0.06, "battery_failure_ratio": 0.03,
            "energy_security_scenario": "normal",
            "enable_geopolitical_shock": False,
        },
        "Reykjavik Storm Test": {
            "country": "Iceland", "city": "Reykjavik", "population": 139000,
            "scenario_key": "storm", "temperature": 5, "wind_speed": 22.0,
            "solar_radiation": 160, "precipitation": 120, "humidity": 88,
            "solar_failure_ratio": 0.20, "wind_failure_ratio": 0.12, "hydro_failure_ratio": 0.10, "battery_failure_ratio": 0.06,
            "energy_security_scenario": "normal",
            "enable_geopolitical_shock": False,
        },
    }
    ...
    energy_security_scenario = preset.get("energy_security_scenario", energy_security_scenario)
...
st.caption(TAIVAS_VERSION + " • Testing version for external evaluation; decision-support only")
...
with top_right:
    ...
    c1, c2 = st.columns(2)
    with c1:
        mini_card("Product Tier", PRODUCT_TIERS[product_tier]["label"])
...
def render_energy_mix_workspace():
    page_question(tr("tabs")[0], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
def render_scenario_comparison_workspace():
    page_question(tr("tabs")[1], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
    ...
    with left:
        ...
        render_delta_chart(delta_df, tr)
    with right:
    ...
    render_critical_load_chart(critical_load_df, tr)
    st.dataframe(critical_load_df, use_container_width=True, hide_index=True)


def render_stress_test_workspace():
    page_question(tr("tabs")[2], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
def render_ai_recommendation_workspace():
    page_question(tr("tabs")[3], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
def render_energy_security_workspace():
    page_question(tr("tabs")[4], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
    row1 = st.columns(4)
...
def render_survival_timeline_workspace():
    page_question(tr("tabs")[5], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
def render_visual_simulator_workspace():
    page_question(tr("tabs")[6], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
def render_concept_lab_workspace():
    page_question(tr("tabs")[7], tr, PAGE_QUESTIONS, st.session_state.get("ui_lang", "English"))
...
# -----------------------------------------------------------------------------
# V8 Product Tier Workspace Layer
# Product tier controls which workspace paths are visible in the SaaS prototype.
# -----------------------------------------------------------------------------
st.markdown("---")
st.subheader("Product Tier Mode")
render_product_tier_panel(product_tier)

st.subheader("Workspace Mode")
available_workspaces = PRODUCT_TIERS[product_tier]["workspaces"]
workspace_mode = st.radio(
    "Choose how you want to use TAIVAS",
    available_workspaces,
    horizontal=True,
    help="Demo is limited to executive review. Pro adds analyst testing. Enterprise includes concept lab and deployment/governance framing.",
)
