...
def _row_get_any(row, candidates, default=0):
    """Safe row value getter for translated / normalized dataframe columns."""
    try:
        keys = list(row.index)
    except Exception:
        return default
    ...
    normalized = {
        str(k).strip().lower().replace(" ", "_").replace("(", "").replace(")", "").replace("%", "pct"): k
        for k in keys
    }
    for c in candidates:
        n = str(c).strip().lower().replace(" ", "_").replace("(", "").replace(")", "").replace("%", "pct")
        if n in normalized:
            return row[normalized[n]]
...
def render_energy_contribution_panel(df):
    ...
    if df is None or getattr(df, "empty", True):
        st.info("Energy contribution panel is unavailable for this run.")
        return

    for _, row in df.iterrows():
        source = _row_get_any(row, [
            tr("source"), "Source", "source", "energy_source", "Energy Source", "能源來源"
        ], "Unknown")

        share = _safe_float_value(_row_get_any(row, [
            tr("current_share"), "Current Share (%)", "current_share", "current_share_pct", "Actual Mix (%)", "actual_mix_pct"
        ], 0))

        avg_share = _safe_float_value(_row_get_any(row, [
            tr("historical_average_share"), "Average Share (%)", "historical_average_share", "average_share_pct", "Installed Mix (%)", "installed_mix_pct"
        ], share))

        current_mw = _safe_float_value(_row_get_any(row, [
            tr("current_output_mw"), "Current Output (MW)", "current_output_mw", "actual_supply_mw", "Actual Supply (MW)"
        ], 0))

        used_mw = _safe_float_value(_row_get_any(row, [
            tr("estimated_use_mw"), "Estimated Use (MW)", "estimated_use_mw", "used_mw"
        ], min(current_mw, current_mw)))

        remaining = _safe_float_value(_row_get_any(row, [
            tr("remaining_margin_mw"), "Remaining Margin (MW)", "remaining_margin_mw", "remaining_mw"
        ], max(current_mw - used_mw, 0)))

        delta = _safe_float_value(_row_get_any(row, [
            tr("change_from_normal"), "Change from Normal (%)", "change_from_normal", "change_from_normal_pct", "Delta (%)"
        ], 0))
...
def render_trend_estimate_panel(trend_df, multistep_df, trend_meta):
    if trend_df is None:
        st.info('Trend disabled (clean version).')
        return
...
trend_estimate_df = None
multistep_forecast_df = None
trend_meta = {}
...
def render_energy_mix_workspace():
    ...
    # Trend disabled
    st.info('Trend module disabled (clean version).')
