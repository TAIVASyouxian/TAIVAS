...
# These modules are optional extensions in the current repository snapshot. The
# manuscript-aligned energy-balance core does not depend on them. Guarded imports
# keep the official app usable without inventing replacement scientific logic.
OPTIONAL_COMPONENT_WARNINGS = []

try:
    from modules.recommendations import recommendation_lines
except Exception as exc:
    OPTIONAL_COMPONENT_WARNINGS.append(f"Rule-based recommendation extension unavailable: {exc}")

    def recommendation_lines(results, energy_security_scenario):
        return [
            "The optional recommendation extension is unavailable. Review the modeled energy gap, "
            "battery state, and scenario assumptions with a qualified professional."
        ]
...
try:
    ...
    OPTIONAL_COMPONENT_WARNINGS.append(f"Energy-security extension unavailable: {exc}")
    ENERGY_SECURITY_SCENARIOS = {"normal": {"label": "Unavailable extension"}}

    def apply_energy_security_layer(*, base_results, **kwargs):
        unchanged = dict(base_results)
        unchanged.update({
            "import_disruption_score": 0.0,
            "fuel_cost_stress": 0.0,
            "reserve_days_remaining": 0.0,
            "recovery_time_estimate": 0.0,
            "energy_security_extension_available": False,
        })
        return unchanged, {"status": "unavailable", "calculation_applied": False}


try:
    from modules.survival_timeline import simulate_survival_timeline
except Exception as exc:
    OPTIONAL_COMPONENT_WARNINGS.append(f"Survival-timeline extension unavailable: {exc}")

    def simulate_survival_timeline(**kwargs):
        return {
            "hours_until_shortfall": "Unavailable",
            "hours_until_critical_failure": "Unavailable",
            "survival_mode_duration": "Unavailable",
            "rows": [],
...
try:
    ...
    CONCEPT_LAB_AVAILABLE = True
except Exception as exc:
    CONCEPT_LAB_AVAILABLE = False
    OPTIONAL_COMPONENT_WARNINGS.append(f"Concept Lab extension unavailable: {exc}")

    def _render_unavailable_concept(*args, **kwargs):
        st.warning("Concept Lab is unavailable in this repository snapshot. Core simulation results are unaffected.")

    render_thermal_principle_simulation = _render_unavailable_concept
    render_phase_change_buffer_concept = _render_unavailable_concept
    render_ground_thermal_sink_concept = _render_unavailable_concept
    render_distributed_thermal_control_concept = _render_unavailable_concept
    render_distributed_harvesting_buffering_concept = _render_unavailable_concept
...
2. Energy System Layer
   Purpose: demand, supply, storage, energy gap, External Support Need Proxy, renewable contribution.
...
- Scenario Resilience Summary: System Performance Score, battery reserve, renewable supply,
  backup support need.
...
- Prefer system-oriented names:
  ...
  Scenario Impact Field, Energy Stress Chain, Resilience State, External Support Need Proxy,
...
TAIVAS_PHASE1_MODEL_BOUNDARY = (
    "Deterministic exploratory output only. Parameters may be author-defined and uncalibrated. "
    "This output is not observed data, historical validation, a forecast, or authorization for "
    "operational action. Human review is required."
)
...
I18N = {
    "English": {
        ...
        "status_eff": "System Performance Status",
        "status_grid": "External Support Need Status",
...
def format_system_performance_score(value) -> str:
    """Format the manuscript-defined score without misreporting zero demand."""
...
def compute_energy_supply(inputs, scenario_key: str, failure_ratios: dict, reserve_recovery_lag_days: int):
    ...
    # PHASE 1 ALIGNMENT START
    # The official Streamlit entry point delegates to the audited deterministic
    # core. Legacy output aliases are returned by that core for compatibility.
    return compute_energy_supply_core(
        inputs=inputs,
        scenario_key=scenario_key,
        scenario=scenario,
        failure_ratios=failure_ratios,
        reserve_recovery_lag_days=reserve_recovery_lag_days,
        facility_factor=facility_demand_factor(facility_name),
        demand_per_capita_mw=DEMAND_PER_CAPITA_MW,
        ...
        battery_loss_rate=BATTERY_ROUND_TRIP_LOSS_RATE,
...
def synchronize_result_indicators(results):
    """Recalculate manuscript-defined indicators after optional stress layers."""
    demand_mw = float(results.get("demand", 0.0) or 0.0)
    energy_gap_mw = max(float(results.get("shortfall", 0.0) or 0.0), 0.0)
    score = calculate_system_performance_score(demand_mw, energy_gap_mw)
    support_proxy = safe_div(energy_gap_mw, demand_mw) * 100.0 if demand_mw > 0 else 0.0
    score_value = round(score, 2) if score is not None else None

    # Legacy aliases remain for backward-compatible consumers and exports.
    results["system_efficiency"] = score_value
    results["system_performance_score"] = score_value
    results["grid_dependency"] = round(support_proxy, 2)
    results["external_support_need_proxy"] = round(support_proxy, 2)
    results["unmet_demand_ratio"] = round(support_proxy / 100.0, 6) if demand_mw > 0 else 0.0
    results["risk_tier"] = calculate_risk_tier(energy_gap_mw, demand_mw)
    return results
...
def apply_geopolitical_shock_to_results(results, shock):
    ...
    # A disabled extension must be a true pass-through. Previously, an existing
    # energy gap reduced battery state even when the selected event was "None".
    # This guard keeps the default official result identical to the core output.
    if shock.get("event_type") == "None" or (supply_penalty == 0.0 and demand_penalty == 0.0):
        updated["geopolitical_event_type"] = shock.get("event_type", "None")
        updated["geopolitical_risk_level"] = shock.get("risk_level", "Low")
        ...
        updated["geopolitical_battery_stress_energy_mwh"] = 0.0
        return updated
    ...
    system_performance_score = calculate_system_performance_score(adjusted_demand, adjusted_shortfall)
    external_support_need_proxy = safe_div(adjusted_shortfall, adjusted_demand) * 100 if adjusted_demand > 0 else 0.0
    # The retained 8% geopolitical stress convention is converted from the MW
    # gap to MWh over the explicit manuscript-defined one-hour interval.
    battery_stress_energy_mwh = adjusted_shortfall * SIMULATION_INTERVAL_HOURS * 0.08
    ...
    updated["grid_dependency"] = round(external_support_need_proxy, 2)
    updated["external_support_need_proxy"] = round(external_support_need_proxy, 2)
    updated["system_efficiency"] = round(system_performance_score, 2) if system_performance_score is not None else None
    updated["system_performance_score"] = updated["system_efficiency"]
    updated["battery_levels"] = round(max(0.0, original_battery - battery_stress_energy_mwh), 2)
    updated["geopolitical_battery_stress_energy_mwh"] = round(battery_stress_energy_mwh, 4)
...
def render_visual_metric_cards(profile):
    st.markdown(f"""
        <div class="flow-grid">
          ...
          <div class="flow-card {_risk_color_class(profile['grid_stress'])}"><div class="flow-label">External Support Need Proxy</div><div class="flow-value">{profile['grid_stress']}%</div></div>
...
def render_scenario_visual_map(visual_scenario, profile):
    ...
    if visual_scenario == "Typhoon Impact":
        ...
        """
        # UI-ONLY CHANGE END
        caption = "External shock travels through fuel markets, import routes, logistics, and the External Support Need Proxy before appearing as shortfall."
    st.markdown(f"""
    ...
    with left:
        ...
        if visual_scenario == "Geopolitical Shock":
            ...
            chain_df = pd.DataFrame([
                ...
                {"Stage": "External Support Need Proxy", "Signal": f"{profile['grid_stress']}%"},
...
I18N["繁體中文"].update({
    ...
    "export_report": "匯出 / 報告",
    "basic_setup_help": "從這裡開始。選擇地點、設施類型與人口。",
    "scenario_help": "選擇想測試的極端天氣類型。",
    ...
    "critical_breakdown": "關鍵負載拆解",
    "timeline_chart": "時間軸圖表",
    "timeline_table": "逐時資料表",
    "reserve_days": "備援天數",
    "shortfall_hour": "供電缺口時間",
    "critical_failure_hour": "關鍵失效時間",
    "solar_capacity": "太陽能容量",
    "wind_capacity": "風力容量",
    "geothermal_capacity": "地熱容量",
    "hydro_capacity": "水力容量",
    ...
    "temperature": "溫度",
    "wind_speed": "風速",
    "solar_radiation": "日照強度",
    "precipitation": "降雨量",
    "humidity": "濕度",
    "solar_failure_ratio": "太陽能失效比例",
    "wind_failure_ratio": "風力失效比例",
    "geothermal_failure_ratio": "地熱失效比例",
    "hydro_failure_ratio": "水力失效比例",
    "battery_failure_ratio": "電池失效比例",
    "energy_security_scenario": "能源安全情境",
    ...
    "change_from_normal": "相較正常差異（%）",
    "reserve_outlook": "備援判讀",
    "historical_average_supply": "平均再生能源供應",
    "current_renewable_supply": "目前再生能源供應",
    "estimated_renewable_use": "估計再生能源使用量",
    "remaining_battery_reserve": "剩餘電池備援",
    "estimated_refill_time": "預估回補時間",
    "no_surplus": "目前沒有多餘供應",
    "hours_short": "小時",
...
with st.sidebar:
    ...
    scenario_explanations = {
        ...
        "storm": "Storm: renewable generation and infrastructure stability may be affected. Solar may decrease and wind may become unstable. Watch first: Renewable Supply, Final Supply, and External Support Need Proxy.",
    ...
    battery_capacity = capacity_panel.slider(tr("battery_capacity"), 0, 1000, int(clamp((uploaded_profile["battery_capacity"] if (use_uploaded and uploaded_profile) else 180), 0, 1000)), 10, help="Battery Capacity (MWh): how much stored backup energy is available.")
    capacity_panel.caption("Battery Capacity is stored energy measured in MWh.")
    ...
    # EI.A reviewer concern: expose research detail without increasing Beginner Mode complexity.
    research_mode = advanced_panel.toggle(
        "Research Mode",
        value=False,
        help="Shows methodology, reproducibility, model-boundary, and evaluation-transparency details. It does not change simulation calculations or outputs.",
    )
    advanced_panel.caption("Research Mode changes presentation only; the same simulation engine and scenario results are retained.")
...
SCENARIO_LOGIC_GUIDE = {
    ...
    "storm": {
        ...
        "Watch first": "Renewable Supply, Final Supply, and External Support Need Proxy.",
    },
    "cold_wave": {
...
results = synchronize_result_indicators(results)
...
baseline_results = synchronize_result_indicators(baseline_results)
        ...
        <div class="emergency-summary primary-brief">
          ...
          <div class="emergency-grid">
            ...
            <div class="emergency-item">
              ...
              <div class="emergency-text">Review battery reserve, energy gap, and the External Support Need Proxy first.</div>
            </div>
          </div>
        </div>
        """,
...
def decision_brief_message():
    ...
    return (
        ...
        f"Current modeled energy gap is {gap:.2f} MW and the External Support Need Proxy is {grid_need:.2f}%. "
        "Review battery reserve, energy gap, and external-support assumptions before drawing conclusions or preparing an illustrative communication draft."
    )
...
def explainable_ai_rows():
    ...
    if results.get("shortfall", 0) > 0:
        ...
        reason = "The External Support Need Proxy is elevated, indicating a larger modeled unmet-demand share under the selected scenario."
    ...
    next_review = "Review the External Support Need Proxy, battery reserve, uploaded data quality, and storage assumptions before drawing conclusions."
    return pd.DataFrame([
...
def generate_agentic_advisory(simulation_results, context):
    ...
    system_performance_score_raw = simulation_results.get("system_performance_score", simulation_results.get("system_efficiency"))
    system_efficiency = float(system_performance_score_raw or 0.0)
    system_performance_score_text = format_system_performance_score(system_performance_score_raw)
    ...
    if shortfall > 0:
        ...
        detected_issue = "The system suggests the External Support Need Proxy is elevated under the selected scenario."
    else:
    ...
    if grid_dependency >= 10:
        recommended_actions.append("Suggested review action: review the External Support Need Proxy and external-supply assumptions.")
...
def render_operational_monitoring_layer():
    ...
    cols[3].metric("External Support Need Proxy", f'{monitoring["grid_dependency_current"]:.2f}%')
...
def build_audit_trail_record():
    ...
    return {
        ...
        "facility_type": facility_type,
        "scenario": {
            "weather": scenario_key,
            "weather_multipliers": dict(SCENARIOS.get(scenario_key, SCENARIOS["normal"])),
        ...
        "simulation_methodology": {
            ...
            "power_unit": "MW",
            "battery_state_and_transfer_unit": "MWh",
            "system_performance_score_definition": "Percentage of modeled demand served.",
            "external_support_need_proxy_definition": "Unmet-demand ratio expressed as a percentage. It does not measure actual imported grid electricity.",
        ...
        "model_boundary": TAIVAS_PHASE1_MODEL_BOUNDARY,
        "optional_extension_status": {
            "warnings": list(OPTIONAL_COMPONENT_WARNINGS),
            "core_simulation_affected": False,
        },
    }
...
# PHASE 1 ALIGNMENT START
# Override the legacy/malformed hero strings with the submitted positioning.
if st.session_state.get("ui_lang", "English") == "繁體中文":
    hero_title = "TAIVAS 能源韌性模擬器"
    hero_subtitle = "在災害發生前，評估極端天氣下可能出現的能源缺口與韌性風險。"
    hero_kicker = "TAIVAS 協助使用者快速檢視能源缺口、備援需求與初步應對行動。"
...
if OPTIONAL_COMPONENT_WARNINGS:
    with st.expander("Deployment component status", expanded=False):
        st.warning(
            "Optional extension modules are unavailable in this repository snapshot. "
            "The manuscript-aligned core energy-balance simulation remains available."
        )
        for component_warning in OPTIONAL_COMPONENT_WARNINGS:
            st.caption(component_warning)
if beginner_mode:
...
def render_kpi_reference_panel():
    ...
    st.subheader("Key Performance Indicator (KPI) Reference")
    st.caption("Units: megawatt (MW), megawatt-hour (MWh), hour, day, and percent (%). KPI definitions are grounded in existing TAIVAS outputs. Unsupported social, political, or budget-performance KPIs are intentionally not included.")
    ...
    with method_cols[0]:
        st.markdown(
            ...
            **Input sources**
            ...
            - Optional uploaded comma-separated values (CSV) rows.
...
def render_research_transparency_panel():
    # EI.A reviewer concerns: black-box simulation, AI ambiguity, reproducibility, and evaluation evidence.
    """Research presentation layer only; reads existing model definitions and outputs."""
    st.subheader("Research Transparency Mode")
    st.caption(
        "This mode opens the existing simulation pipeline for methodological review. "
        "It does not change inputs, formulas, scenarios, KPIs, or results."
    )

    pipeline_tab, boundary_tab, reproducibility_tab, evaluation_tab, abbreviation_tab = st.tabs(
        ["Simulation Pipeline", "Artificial Intelligence (AI) and Rule Boundaries", "Reproducibility", "Evaluation Status", "Abbreviations"]
    )

    with pipeline_tab:
        st.markdown(
            "**Inputs -> Scenario Logic -> Demand and Supply -> Storage -> Energy Gap -> "
            "Resilience Indicators -> Risk Classification -> Rule-Based Explanation**"
        )
        pipeline_rows = [
            {
                "Stage": "Inputs",
                "Required information": "Location, population/facility, weather, installed capacity, storage, and failure assumptions",
                "Method": "Built-in references, user controls, or optional uploaded comma-separated values (CSV)",
                "Main limitation": "Input quality and completeness directly affect interpretation",
            },
            {
                "Stage": "Scenario logic",
                "Required information": "Selected scenario",
                "Method": "Predefined demand, renewable-source, and battery multipliers",
                "Main limitation": "Simplified stress representation, not a weather forecast",
            },
            {
                "Stage": "Demand estimation",
                "Required information": "Population, facility type, scenario, and temperature",
                "Method": "Population demand basis multiplied by weather stress and facility factors",
                "Main limitation": "Not a metered or utility-grade load forecast",
            },
            {
                "Stage": "Supply estimation",
                "Required information": "Capacity, resource inputs, scenario factors, and failure ratios",
                "Method": "Source-specific solar, wind, hydro, and geothermal availability formulas",
                "Main limitation": "No live dispatch, weather, hydrology, or market integration",
            },
            {
                "Stage": "Storage and final supply",
                "Required information": "Battery capacity/current level, losses, dispatch limit, and grid-support assumption",
                "Method": "MW charge/discharge power converted to MWh over an explicit 1-hour interval, followed by bounded storage and supply aggregation",
                "Main limitation": "Not a battery-management-system or grid-dispatch model",
            },
            {
                "Stage": "Energy gap and indicators",
                "Required information": "Demand, final supply, renewable supply, and battery level",
                "Method": "Deterministic arithmetic and threshold-based risk classification",
                "Main limitation": "Comparative scenario indicators, not confirmed outage probability",
            },
            {
                "Stage": "Explanation and suggested review",
                "Required information": "Existing outputs and baseline differences",
                "Method": "Transparent conditional rules and text templates",
                "Main limitation": "Decision support only; qualified human interpretation remains required",
            },
        ]
    ...
    with boundary_tab:
        boundary_rows = [
            {"Component": "Core simulation", "Implementation": "Deterministic Python formulas", "Runtime artificial intelligence (AI) model": "No", "Meaning": "The same inputs and configuration produce the same modeled outputs."},
            {"Component": "Scenario application", "Implementation": "Predefined multipliers and availability assumptions", "Runtime artificial intelligence (AI) model": "No", "Meaning": "Scenario effects are explicit and inspectable."},
            {"Component": "Risk classification", "Implementation": "Threshold-based rule engine", "Runtime artificial intelligence (AI) model": "No", "Meaning": "Risk labels summarize modeled energy-gap severity."},
            {"Component": "Advisory and explanation", "Implementation": "Conditional rules and text templates", "Runtime artificial intelligence (AI) model": "No", "Meaning": "Existing outputs are translated into plain-language interpretation and variables for further testing."},
            {"Component": "User interpretation", "Implementation": "Human review across four presentation roles", "Runtime artificial intelligence (AI) model": "No", "Meaning": "Role selection changes explanation only, never the simulation result."},
        ]
        st.dataframe(pd.DataFrame(boundary_rows), use_container_width=True, hide_index=True)
        st.info(
            "The current main application does not contain a runtime generative-AI model. "
            "Its advisory layer is a transparent rule-based interpretation layer. Any academic use of "
            "'AI-assisted' or 'AI-supported' should be limited to separately documented assistance and must not imply autonomous prediction or control."
        )

    with reproducibility_tab:
        st.caption("Units used below: megawatt (MW), megawatt-hour (MWh), hour (h), megawatt per person (MW/person), and dimensionless ratio.")
        parameter_rows = [
            {"Parameter": "Demand per capita", "Value": DEMAND_PER_CAPITA_MW, "Unit": "MW/person", "Used by": "Base demand", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Facility demand factors", "Value": str(FACILITY_DEMAND_FACTORS), "Unit": "ratio", "Used by": "Demand", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Solar coefficient", "Value": SOLAR_EFFICIENCY, "Unit": "ratio", "Used by": "Solar supply", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Wind coefficient", "Value": WIND_EFFICIENCY, "Unit": "ratio", "Used by": "Wind supply", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Geothermal availability", "Value": GEOTHERMAL_AVAILABILITY_FACTOR, "Unit": "ratio", "Used by": "Geothermal supply", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Hydropower availability", "Value": "clamp(0.45 + precipitation/500 x 0.35, 0.15, 0.95)", "Unit": "ratio", "Used by": "Hydropower supply", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Battery charge share", "Value": 0.30, "Unit": "ratio", "Used by": "Surplus charging", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Battery dispatch-capacity share", "Value": 0.35, "Unit": "ratio/interval", "Used by": "Discharge limit", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Battery transfer loss", "Value": BATTERY_ROUND_TRIP_LOSS_RATE, "Unit": "ratio", "Used by": "Charge/discharge energy loss", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
            {"Parameter": "Simulation interval", "Value": SIMULATION_INTERVAL_HOURS, "Unit": "h", "Used by": "MW-to-MWh conversion", "Evidence status": "Author-defined implementation convention; explicit in the submitted methodology."},
            {"Parameter": "Risk thresholds", "Value": "<5% Elevated; 5-<15% High; >=15% Critical", "Unit": "unmet-demand ratio", "Used by": "Risk Tier", "Evidence status": "Author-defined exploratory assumption; not empirically calibrated."},
        ]
        st.markdown("##### Core parameter reference")
        ...
        for scenario_name, factors in SCENARIOS.items():
            scenario_rows.append({"Scenario": scenario_name, **factors})
        st.markdown("##### Scenario-factor reference")
    ...
    with evaluation_tab:
        evaluation_rows = [
            {"Evidence area": "Formal reviewer or participant count", "Current documented status": "Not encoded in the prototype", "Required reporting action": "Report only the actual number supported by study records."},
            {"Evidence area": "Professional background", "Current documented status": "Not encoded in the prototype", "Required reporting action": "Describe verified participant roles and recruitment criteria."},
            {"Evidence area": "Review protocol and usability objectives", "Current documented status": "Requires study documentation", "Required reporting action": "State tasks, questions, observation method, and success criteria."},
            {"Evidence area": "Findings and interface changes", "Current documented status": "Code history shows iterative UI refinement; no formal causal claim is encoded", "Required reporting action": "Link each reported change to recorded feedback or an identified design limitation."},
            {"Evidence area": "External engineering validation", "Current documented status": "Not completed", "Required reporting action": "Present as a limitation; do not imply validated infrastructure performance."},
            {"Evidence area": "Regression checks", "Current documented status": "Lightweight automated tests are present in the repository", "Required reporting action": "Report only test runs that were actually executed and observed."},
        ]
        st.dataframe(pd.DataFrame(evaluation_rows), use_container_width=True, hide_index=True)
        st.warning(
            "This panel is a transparency aid, not evaluation evidence. It intentionally does not invent participant counts, citations, validation results, or scientific claims."
        )
        st.markdown(
            "**Implementation-grounded contribution:** TAIVAS combines a shared scenario engine, "
            "baseline comparison, progressive disclosure, role-sensitive explanation, model-boundary reminders, "
            "and transparent rule-based interpretation in one resilience-understanding prototype. "
            "Claims of novelty still require comparison with cited prior work in the manuscript."
        )

    with abbreviation_tab:
        abbreviation_rows = [
            {"Abbreviation": "AI", "Full term": "Artificial intelligence", "TAIVAS context": "Used only when clarifying AI-related claims; the current main application has no runtime generative-AI model."},
            {"Abbreviation": "API", "Full term": "Application programming interface", "TAIVAS context": "Future integration terminology; no external API is required by the current core simulation."},
            {"Abbreviation": "CSV", "Full term": "Comma-separated values", "TAIVAS context": "Optional uploaded data and downloadable tabular reports."},
            {"Abbreviation": "GIS", "Full term": "Geographic information system", "TAIVAS context": "Boundary term used to clarify that the visual scenario layer is not a GIS product."},
            {"Abbreviation": "JSON", "Full term": "JavaScript Object Notation", "TAIVAS context": "Structured audit export format."},
            {"Abbreviation": "KPI", "Full term": "Key performance indicator", "TAIVAS context": "Modeled demand, supply, storage, gap, duration, and risk indicators."},
            {"Abbreviation": "MW", "Full term": "Megawatt", "TAIVAS context": "Power used for demand, supply, capacity, and energy-gap displays."},
            {"Abbreviation": "MWh", "Full term": "Megawatt-hour", "TAIVAS context": "Energy used for battery and reserve displays."},
            {"Abbreviation": "PCM", "Full term": "Phase-change material", "TAIVAS context": "Concept Lab thermal-storage visualization only; not validated hardware."},
            {"Abbreviation": "PDF", "Full term": "Portable Document Format", "TAIVAS context": "Downloadable report format in the utility toolkit."},
            {"Abbreviation": "PPT/PPTX", "Full term": "PowerPoint presentation / PowerPoint Open XML presentation", "TAIVAS context": "Rapid briefing-deck export format."},
            {"Abbreviation": "UI", "Full term": "User interface", "TAIVAS context": "Presentation layer; separate from the simulation and rule-based explanation layers."},
        ]
        st.dataframe(pd.DataFrame(abbreviation_rows), use_container_width=True, hide_index=True)
        st.caption("For the manuscript, each abbreviation must still be defined independently at first use in the abstract, main text, figures, tables, and captions.")
...
def render_resilience_storytelling():
    ...
    st.markdown(
        ...
        External Support Need Proxy changes by <b>{grid_delta_pct:+.1f} percentage points</b>,
...
def render_system_explanation_panel():
    ...
    if results["grid_dependency"] > baseline_results["grid_dependency"]:
        explanations.append("External Support Need Proxy has increased, indicating a larger modeled unmet-demand share.")
    else:
        explanations.append("External Support Need Proxy has not increased under the selected scenario assumptions.")
...
def render_scenario_comparison_workspace():
    ...
    with st.expander("Detailed Technical Comparison", expanded=False):
        ...
        with right:
            ...
            grid_col = tr("grid") if tr("grid") in scenario_df.columns else "External Support Need Proxy"
...
def render_distributed_thermal_control_concept(
    ...
    .route-link {{position:absolute; height:8px; border-radius:999px; background:#64748B; opacity:.62; overflow:hidden; transition:opacity .45s ease, box-shadow .45s ease;}}
    .route-link.blue {{background:rgba(14,165,233,.68); box-shadow:0 0 14px rgba(14,165,233,.14);}}
    .route-link.red {{background:rgba(225,29,72,.58); opacity:.68; box-shadow:0 0 12px rgba(225,29,72,.10);}}
    .route-link.blue::after,.route-link.red::after {{content:""; position:absolute; inset:0; width:45%; border-radius:999px; background:linear-gradient(90deg, transparent, rgba(255,255,255,.42), transparent); animation:routing-soft-flow 6.8s ease-in-out infinite;}}
    .route-link.red::after {{animation-duration:8.8s; opacity:.46;}}
    .route-link:hover {{opacity:.92; box-shadow:0 0 18px rgba(125,211,252,.22);}}
    ...
    .route-node {{position:absolute; width:42px; height:42px; border-radius:999px; background:#1E293B; border:3px solid #94A3B8; transition:border-color .35s ease, box-shadow .35s ease, transform .35s ease;}}
    .route-node:hover {{border-color:#BAE6FD; box-shadow:0 0 18px rgba(125,211,252,.20); transform:scale(1.035);}}
    .route-core {{position:absolute; left:45%; top:43%; width:60px; height:60px; border-radius:999px; background:#5EEAD4; border:3px solid #E0F2FE; display:grid; place-items:center; font-weight:900; color:#06251F; animation:core-support-breathe 5.8s ease-in-out infinite;}}
    ...
    .routing-side h4{{margin:.1rem 0 .65rem 0; color:#F8FAFC; font-size:1.04rem;}} .routing-brief{{border-left:3px solid rgba(56,189,248,.70); padding-left:.72rem; color:#CBD5E1; line-height:1.5; font-size:.9rem; margin:.75rem 0 .9rem 0; transition:color .35s ease, border-color .35s ease;}} .routing-brief:hover{{color:#E5E7EB; border-color:rgba(125,211,252,.90);}} .routing-boundary{{padding:.72rem .78rem; border-radius:12px; background:rgba(245,158,11,.10); border:1px solid rgba(245,158,11,.24); color:#FDE68A; font-size:.82rem; line-height:1.45; margin-top:.8rem;}}
    @keyframes routing-soft-flow {{0%,100%{{transform:translateX(-120%); opacity:.20;}}50%{{opacity:.60;}}100%{{transform:translateX(260%);}}}}
    @keyframes core-support-breathe {{0%,100%{{box-shadow:0 0 0 rgba(94,234,212,0);}}50%{{box-shadow:0 0 24px rgba(94,234,212,.22);}}}}
...
def render_distributed_harvesting_buffering_concept(
    ...
    .source-node,.buffer-node,.output-node {{position:absolute; border-radius:18px; border:2px solid rgba(96,165,250,.70); background:rgba(15,23,42,.72); display:grid; place-items:center; text-align:center; color:#F8FAFC; font-weight:900; transition:box-shadow .45s ease, border-color .45s ease, transform .45s ease;}}
    .source-node:hover,.buffer-node:hover,.output-node:hover {{box-shadow:0 0 18px rgba(125,211,252,.18); transform:translateY(-1px);}}
    ...
    .buffer-node {{left:39%; top:32%; width:190px; height:128px; animation:buffer-state-breathe 6.4s ease-in-out infinite;}} .output-node {{right:9%; top:38%; width:140px; height:96px; border-color:#34D399; animation:output-stability-glow 7.2s ease-in-out infinite;}}
    .source-flow {{position:absolute; height:9px; border-radius:999px; transform-origin:left center; opacity:.78; overflow:hidden;}}
    .source-flow::after {{content:""; position:absolute; inset:0; width:42%; border-radius:999px; background:linear-gradient(90deg, transparent, rgba(255,255,255,.40), transparent); animation:source-flow-pulse 6.8s ease-in-out infinite;}}
    .source-flow.fsolar{{left:20%; top:27%; width:22%; transform:rotate(22deg); background:rgba(245,158,11,.76);}} .source-flow.fwind{{left:20%; top:53%; width:23%; transform:rotate(-6deg); background:rgba(14,165,233,.76);}} .source-flow.fhydro{{left:20%; top:78%; width:25%; transform:rotate(-28deg); background:rgba(37,99,235,.76);}} .source-flow.fout{{left:58%; top:49%; width:24%; background:rgba(22,163,74,.74);}}
    ...
    .buffering-side h4{{margin:.1rem 0 .65rem 0; color:#F8FAFC; font-size:1.04rem;}} .buffering-brief{{border-left:3px solid rgba(56,189,248,.70); padding-left:.72rem; color:#CBD5E1; line-height:1.5; font-size:.9rem; margin:.75rem 0 .9rem 0; transition:color .35s ease, border-color .35s ease;}} .buffering-brief:hover{{color:#E5E7EB; border-color:rgba(125,211,252,.90);}} .buffering-boundary{{padding:.72rem .78rem; border-radius:12px; background:rgba(245,158,11,.10); border:1px solid rgba(245,158,11,.24); color:#FDE68A; font-size:.82rem; line-height:1.45; margin-top:.8rem;}}
    @keyframes source-flow-pulse {{0%,100%{{transform:translateX(-120%); opacity:.20;}}50%{{opacity:.58;}}100%{{transform:translateX(260%);}}}}
    @keyframes buffer-state-breathe {{0%,100%{{box-shadow:0 0 0 rgba(96,165,250,0);}}50%{{box-shadow:0 0 24px rgba(96,165,250,.16);}}}}
    @keyframes output-stability-glow {{0%,100%{{box-shadow:0 0 0 rgba(52,211,153,0);}}50%{{box-shadow:0 0 20px rgba(52,211,153,.18);}}}}
...
def render_concept_lab_workspace():
    ...
    if not CONCEPT_LAB_AVAILABLE:
        st.warning(
            "Concept Lab is unavailable because its optional component file is not present in this "
            "repository snapshot. The core simulation and calculated results are unaffected."
        )
        return
...
def render_main_system_status():
    # Energy System Layer: demand, supply, energy gap, and System Performance Score.
    ...
    with status_left:
        ...
        metric_cols[1].metric(
            tr("eff"),
            format_system_performance_score(results.get("system_performance_score")),
            help="Percentage of modeled demand served. Not applicable when modeled demand is zero.",
        )
    with status_right:
    ...
    st.info(TAIVAS_PHASE1_MODEL_BOUNDARY)
    # UI REFINEMENT END
...
def rank_shortfall_drivers():
    ...
    if efficiency_delta < 0:
        drivers.append({
            "Driver": "System Performance Score drop",
            "Impact": abs(efficiency_delta),
            "Evidence": f"System Performance Score changed by {efficiency_delta:+.1f} percentage points.",
        })
...
def render_product_advanced_analytics():
    ...
    if research_mode:
        render_research_transparency_panel()
