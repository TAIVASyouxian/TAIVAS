...
ANALYTICS_SOURCE = (ROOT / "pages" / "99_Analytics_Dashboard.py").read_text(
    encoding="utf-8"
)
...
class DeployedUiAlignmentTests(unittest.TestCase):
    ...
    def test_public_positioning_and_observed_comparison_labels(self):
        for expected in (
            "TAIVAS Energy Resilience Stress-Screening Tool",
            "TAIVAS: A Transparent Scenario-Based Energy Resilience "
            "Stress-Screening Tool for Extreme-Weather Decision Support",
            "Step 3 - Stress-Test Energy Resilience",
        ...
        for obsolete_visible_label in (
            '"Empirical Validation Mode"',
            '"Empirical Validation Snapshot"',
            '"Model Assumptions and Limitations"',
            "Step 3 - Simulate Energy Resilience",
        ):
            self.assertNotIn(obsolete_visible_label, SOURCE)
        self.assertIn("Free-text responses may contain", SOURCE)
        self.assertNotIn("Anonymous feedback only", SOURCE)

    def test_analytics_utility_is_clearly_noncanonical(self):
        self.assertIn("Internal Legacy Testing Analytics", ANALYTICS_SOURCE)
        self.assertIn("Not canonical TAIVAS model output", ANALYTICS_SOURCE)
        self.assertIn("voluntarily entered", ANALYTICS_SOURCE)
        self.assertIn("may contain identifying information", ANALYTICS_SOURCE)
        self.assertNotIn("No names, emails", ANALYTICS_SOURCE)
